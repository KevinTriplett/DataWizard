...
**`operator`**: The human team member whose session created or substantially updated this file. Applied at write-time by the DW instance. Use first name only (e.g. `Andrew`, `Kaliya`, `Jay`). Apply to:
- Session log section files (always)
- Content documents created or substantially updated during a session

This field was not applied to files before Phase 0. Existing files gain it when next touched; no bulk backfill needed.
