...
When conversion scripts are available (see [[Document Converter Guide]]), use them instead of manual conversion. The scripts handle PDF conversion via Docling (including complex layouts with tables, charts, and multi-column documents), DOCX heading detection, YAML frontmatter generation, and shell+sections splitting for large files.

### What to do with files the pipeline can't process

Some files (images, audio, scanned PDFs with no extractable text) can't be meaningfully converted to markdown. These should be:
