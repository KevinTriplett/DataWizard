...
Each project has its own archive folder using the `_Archive` prefix with a project name suffix:

```
_ProjectFolder/_Archive - ProjectName/
```

The `_` prefix sorts the folder to the top of the file explorer and is shell-safe (no escaping needed). The `- ProjectName` suffix makes archives identifiable when searching across the vault. For vault-level files not tied to a specific project, use a root-level archive:
