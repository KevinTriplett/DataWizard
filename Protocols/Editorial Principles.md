...
3. **Create numbered section files.** Use the `1.0+` numbering convention (see File naming in the Conventions Registry). Each section file starts with `*Part of the [[Shell Document Name]]*` and carries YAML frontmatter with `type`, `section`, and `parent` fields.
...
*Extracted from the DataWizard Universal Protocol (section 7.0) in the S182 demolition (D94). Structural conventions live in the [[Conventions Registry]]; field definitions in the [[YAML Schema]].*
